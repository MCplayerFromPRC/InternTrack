...
import {
  CkptInput,
  createCkptMutation,
} from "../checkpoint/create_checkpoint.mutation";
import {
  CkptStepInput,
  createCkptStepMutation,
} from "../ckpt_step/create_ckpt_step.mutation";
import {
  TrainConfigInput,
  createTrainConfigMutation,
} from "../train_config/create_train_config.mutation";
import {
  ResumeCkptInput,
  createResumeCkptMutation,
} from "../resume_ckpt/create_resume_ckpt.mutation";
      ...
      return createRoadmapMutation(
        {
          ...
          resumeCkptList: args.resumeCkptList.map(
            ({ from, to, ...others }) => ({
              _from: from,
              _to: to,
              ...others,
...
export async function createRoadmapMutation(
  ...
  trainConfigList.map((trainConfig) =>
    createTrainConfigMutation(trainConfig, context),
  );
  resumeCkptList.map((resumeCkpt) =>
    createResumeCkptMutation(resumeCkpt, context),
  );
  return {
    ckptList,
    ckptStepList,
